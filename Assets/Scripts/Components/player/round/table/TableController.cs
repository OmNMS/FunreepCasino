    ...
    public class TableController : MonoBehaviour, ITableController
    {
        ...
        public void LastChipDestroy(bool value)
        {
            if(characterTable.currentTableCount > 0)
            {
                ...
                Debug.Log((characterTable.currentTable[characterTable.currentTable.Count - 1].gameObject.GetComponent<ChipGame>().characterTable.currentChipSelected.chipValue));
                int x = int.Parse(Managers.GameManager.Instance.PlacedBetsTxt.text)
                    - characterTable.currentTable[characterTable.currentTable.Count - 1].gameObject.GetComponent<ChipGame>().characterTable.currentChipSelected.chipValue;
                Managers.GameManager.Instance.PlacedBetsTxt.text = x.ToString();
