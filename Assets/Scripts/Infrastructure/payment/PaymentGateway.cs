...
namespace Infrastructure
{
    public class PaymnentGateway : IPayment
    {
        ...
        public IObservable<Unit> PaymentSystem(CharacterTable characterTable)
        {
            ...
            Debug.Log(paymentChipsReturn);

            Managers.GameManager.Instance.balance +=  _payment;
            if (_payment < 0)
            {
                Managers.GameManager.Instance.BalanceTxt.text = Managers.GameManager.Instance.balance.ToString();
            }
